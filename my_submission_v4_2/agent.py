    ...
    async def run(
        ...
        for step in range(1, max_steps + 1):
            ...
            if self._autopilot_queue:
                ...
                # KEY FIX: if autopilot landed in Unknown, abort remaining path
                # to avoid looping through unreliable graph nodes
                if location == "Unknown" and len(self._autopilot_queue) > 0:
                    if verbose:
                        print(f"[AUTOPILOT] Arrived at Unknown — aborting remaining path")
                    self._autopilot_queue.clear()
                ...
                continue
            # ─────────────────────────────────────────────────────────────────
            # NOTE: No Unknown sweep — the LLM handles direction exploration
            # naturally via failed_actions context + suggest_actions tool.
            ...
            # ── Trigger BFS autopilot when stagnant (only from named rooms) ──
            # BFS is unreliable if current_location is Unknown: the server graph
            # may point to Unknown nodes that loop. Only trigger from named rooms.
            if (has_navigate
