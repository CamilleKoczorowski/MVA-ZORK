...
class AgentMemory:
    ...
    def room_key(location: str, obs: str = "") -> str:
        """Unique key per physical room. Unknown rooms are fingerprinted by obs hash."""
    ...
    def update(self, location: str, new_location: str, action: str, obs_before: str,
        ...
        # Use obs-hash key so Unknown rooms don't contaminate each other
    ...
    def is_action_failed(self, location: str, action: str, obs: str = "") -> bool:
    ...
    def get_failed_here(self, location: str, obs: str = "") -> list[str]:
        key = self.room_key(location, obs)
        return sorted(self.failed_actions.get(key, set()))

    def get_context_string(self, location: str, obs: str = "") -> str:
        lines = []
...
class StudentAgent:
    ...
    def __init__(self):
        ...
        # BFS cooldown: after autopilot arrives, give LLM N steps to explore
        self._bfs_cooldown: int = 0
        # V5: Cardinal sweep — try N/S/E/W mechanically on first visit to any named room
        self._sweep_queue: deque[str] = deque()
        self._swept_locations: set[str] = set()
        # V6: Auto-Loot — take key items automatically on first visit to a room
    ...
    async def run(
        ...
        # V5: queue cardinal sweep for the starting room immediately
        self._maybe_queue_sweep(location, observation)
        ...
        for step in range(1, max_steps + 1):
            ...
            if self._autopilot_queue:
                ...
                # When autopilot finishes, set cooldown so LLM can explore the new room
                if not self._autopilot_queue and location != "Unknown":
                    self._bfs_cooldown = 15
                    if verbose:
                        print(f"[BFS] Arrived at {location} — cooldown 15 steps")
                # V5/V6: queue sweep and loot if we just entered a new named room
                self._maybe_queue_sweep(location, new_obs)
            ...
            # ── V5 Cardinal Sweep ─────────────────────────────────────────────
            # On first visit to any named room, exhaust N/S/E/W before the LLM.
            # Guarantees immediate corridor discovery at zero LLM cost.
            if self._sweep_queue:
                ...
                if new_location != location:
                    ...
                    if new_location != "Unknown":
                        ...
                        self._bfs_repeat_count = 0
                        self._last_bfs_path = []
                    # Entered a new room mid-sweep: clear remaining sweep,
                    # BFS autopilot will handle navigation back if needed
                    self._sweep_queue.clear()
                    self._maybe_queue_sweep(new_location, new_obs)
            ...
            # ── V6 Auto-Loot ─────────────────────────────────────────────────
            # On first visit, mechanically take any recognizable item from obs.
            # Fires AFTER sweep so we're still in the same room.
            # Must check loot BEFORE LLM — saves steps on obvious pickups.
            ...
            # ── Smart Reflexion — only on score milestone or every 20 steps ──
            # Reflexion on score change = lessons from a real puzzle breakthrough.
            # Fallback every 20 steps avoids complete silence on score-free games.
            score_just_changed = (
            ...
            # Guard: only from named rooms, BFS not deadlocked, cooldown elapsed
            ...
            # ── Normal LLM step ──────────────────────────────────────────────
            memory_ctx = self.memory.get_context_string(location, observation)
            ...
            if tool_name == "navigate_to_frontier":
                # Generic deadlock guard: refuse BFS from Unknown, repeat, or cooldown
                if location == "Unknown" or self._bfs_repeat_count >= 2 or self._bfs_cooldown > 0:
                    ...
                    fallback = self._find_alternative_action(location, self._last_obs)
            ...
            if tool_name == "play_action":
                ...
                if new_location != location:
                    ...
                    if new_location != "Unknown":
                        ...
                        self._bfs_repeat_count = 0
                        self._last_bfs_path = []
                # Always trigger: obs-hash distinguishes Unknown rooms physically
                self._maybe_queue_sweep(new_location, new_obs)
                self._maybe_queue_loot(new_location, new_obs)
                location = new_location
    ...
    def _build_prompt(self, observation: str, memory_ctx: str) -> str:
        ...
        # 1. FORBIDDEN ACTIONS — always first, maximum LLM attention
        ...
        # 2. Warnings (cycle/stagnation) — urgent signals
        ...
        # 4. Reflexion notes (lessons from score breakthroughs only)
    ...
    def _sanitize_tool_call(
        ...
        if self.memory.is_action_failed(location, action, self._last_obs):
            action = self._find_alternative_action(location, self._last_obs)
            tool_args["action"] = action
    ...
    # Patterns like "Tunnel go east" or "only place is north" or "stairs go up"
    _EXIT_PATTERNS = re.compile(
        r'(north|south|east|west|up|down|northeast|northwest|southeast|southwest|enter|exit)',
        re.IGNORECASE
    )

    def _parse_exits_from_obs(self, obs: str) -> list[str]:
        """Extract directions explicitly mentioned in the observation text."""
        # Only parse lines that look like exit descriptions
        TRIGGER_WORDS = ["go", "way", "lead", "only", "tunnel", "door", "passage",
                         "exit", "stairs", "path", "corridor", "opening"]
        exits = []
        for line in obs.lower().split("\n"):
            if any(t in line for t in TRIGGER_WORDS):
                exits.extend(m.group(1).lower()
                             for m in self._EXIT_PATTERNS.finditer(line))
        return list(dict.fromkeys(exits))  # deduplicate, preserve order

    def _find_alternative_action(self, location: str, obs: str = "") -> str:
        """Find an untried direction, prioritizing known graph exits then obs-parsed exits."""
        # For Unknown rooms: don't use failed_actions (contaminated across rooms)
        ...
        # Second (new): parse exits explicitly mentioned in current observation
        if obs:
            for direction in self._parse_exits_from_obs(obs):
                if direction not in failed:
                    return direction

        # Third: try standard directions in a smart order
    ...
    def _room_key(self, location: str, obs: str = "") -> str:
        """Unique key for a room. For Unknown, use obs hash to distinguish rooms."""
        if not location.startswith("Unknown") or not obs:
            return location
        return f"Unknown:{hash(obs[:80]) & 0xFFFF}"

    def _maybe_queue_sweep(self, location: str, obs: str = "") -> None:
        """
        V5/V7: On first visit to any room, queue N/S/E/W.
        Uses obs-hash to distinguish between different Unknown rooms.
        """
        key = self._room_key(location, obs)
        if key in self._swept_locations:
            return
        self._swept_locations.add(key)
        failed_here = set() if location.startswith("Unknown") else self.memory.failed_actions.get(location, set())
    ...
    # Takeable items — generic Z-machine vocabulary, no game-specific names
    LOOT_KEYWORDS = [
    ...
    def _maybe_queue_loot(self, location: str, observation: str) -> None:
        """
        V6: On first visit to a named room, scan the observation for takeable
        items and queue \'take <item>\' mechanically — no LLM needed.
        Only fires once per room. Items already in failed_actions are skipped.
        """
        key = self._room_key(location, observation)
        if key in self._looted_rooms:
            return
        self._looted_rooms.add(key)
