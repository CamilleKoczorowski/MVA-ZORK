...
class StudentAgent:
    ...
    def __init__(self):
        ...
        self._jericho_valid_ok: bool = True  # disabled if get_valid_actions hangs
    ...
    # Outcome summary: use first line of obs (avoids extra LLM call per step)
    def _outcome_summary(self, obs: str) -> str:
    ...
    async def _on_enter_location(self, client, location: str, obs: str,
        ...
        # Get valid actions from Jericho — guarded with asyncio timeout
        if has_valid_actions and self._jericho_valid_ok:
            try:
                ...
                # Jericho hangs on this game/env — disable for rest of run
                self._jericho_valid_ok = False
                if verbose:
                    print("[ENTRY] get_valid_actions timeout — disabling Jericho valid actions")
    ...
    async def run(self, client, game: str, max_steps: int, seed: int, verbose=False) -> RunResult:
        ...
        for step in range(1, max_steps + 1):
            ...
            while loc.promising:
                ...
                # Log outcome: first meaningful line of obs (no extra LLM call)
                first_line = new_obs.strip().split("\n")[0][:80]
                loc.outcomes[action] = first_line
