...
SYSTEM_PROMPT = """You play text adventures. Explore rooms, pick up items, examine things, maximize score.
...
Other tools: suggest_actions, navigate_to_frontier, inventory, memory, get_map, get_location
...
class StudentAgent:
    ...
    def _build_prompt(self, obs: str, location: str, loc: LocationState) -> str:
        ...
        parts.append(clean)
        parts.append("\n[" + location + " | Score stagnant: " + str(self.memory._no_score_steps) + "]")

        if loc.steps_here >= self.MAX_STEPS_ROOM:
            parts.append("You have been here too long. Move on.")
