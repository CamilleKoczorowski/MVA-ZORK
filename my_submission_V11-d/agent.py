...
SYSTEM_PROMPT = """You are an expert text adventure player. Maximize score by exploring rooms and interacting with objects.

RESPONSE FORMAT (follow exactly):
EXITS: <list exits mentioned in the observation, or "none mentioned">
OBJECTS: <list notable objects, or "none">
THOUGHT: <based on exits and objects, what should I do?>
TOOL: <tool_name>
ARGS: {"action": "..."}

IMPORTANT:
- If the text says "only way to go is east" → ONLY east is valid
- FORBIDDEN actions always fail. Never try them.
- Examine objects you haven't examined yet — this scores points
- Take any items you find
- If stuck: navigate_to_frontier

TOOLS: play_action, suggest_actions, navigate_to_frontier, memory, get_map, inventory, get_location
FORBIDDEN verbs: check, inspect, search, grab, use, go, investigate, get"""
...
class StudentAgent:
    ...
    def _build_prompt(self, obs: str, location: str, loc: LocationState) -> str:
        ...
        if loc.failed:
            parts.append("FORBIDDEN — these FAILED, do NOT retry: " + ", ".join(sorted(loc.failed)))

        parts.append("\n=== ROOM: " + location + " ===")
        parts.append(clean)

        if loc.tried:
            parts.append("\nAlready tried: " + ", ".join(sorted(loc.tried)[:15]))

        exits = self.memory.location_graph.get(location, {})
        if exits:
            exit_strs = [d + " -> " + dest for d, dest in exits.items()]
            parts.append("Known exits: " + ", ".join(exit_strs))

        if self.memory.is_stagnant(10):
            parts.append("\nScore frozen 10+ steps. Try: examine objects, take items, or navigate_to_frontier")
        if loc.steps_here >= self.MAX_STEPS_ROOM:
            parts.append(str(loc.steps_here) + " steps here. Leave this room.")

        parts.append("\nAnalyze: list EXITS and OBJECTS from the text, then decide.")
        return "\n".join(parts)
