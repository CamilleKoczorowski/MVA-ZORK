"""
MCP Server V11b — With get_valid_actions (simple, no threading)

Key changes vs V10:
- REMOVED concurrent.futures threading around get_valid_actions
  (was root cause of hang: thread timeout doesn't kill thread → GIL blocked)
- get_valid_actions is now a simple try/except direct call
- If spaCy works → returns authoritative list (huge advantage)
- If spaCy hangs/fails → returns fallback directions
- Reverse directions in location_graph for better BFS coverage
"""
...
mcp = FastMCP("Text Adventure Server V11b")
...
class GameState:
    ...
    def take_action(self, action: str) -> dict:
        ...
        # Build location graph for BFS (with reverse directions)
    ...
    # -------------------------------------------------------------------------
    # Jericho valid actions — SIMPLE direct call (no threading!)
    # -------------------------------------------------------------------------

    def get_jericho_valid_actions(self) -> list[str]:
        """
        Direct call to Jericho get_valid_actions.
        No threading, no concurrent.futures — just try/except.
        Relies on spaCy being installed + MCP Client timeout being generous (120s).
        """
        SKIP = {"", "yes", "no", "again", "oops", "undo", "restart", "quit",
                "score", "verbose", "save", "restore"}
        try:
            import io
            ...
            if actions:
                return [a for a in actions if a.lower() not in SKIP]
        except Exception:
            pass
...
def get_valid_actions() -> str:
    """
    Get valid actions in current state from Jericho API.
    Returns directions and object interactions separately.
    """
    game = get_game()
    valid = game.get_jericho_valid_actions()
    if not valid:
        return "No valid actions found — use manual exploration."
    ...
    if others:
        lines.append(f"  Actions: {', '.join(others[:20])}")
    return "\n".join(lines)
