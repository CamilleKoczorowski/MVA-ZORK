"""
MCP Server V11d — Fast, no get_valid_actions, WITH text location fallback

V11c bug: removed text fallback from _get_jericho_location() → ALL rooms "Unknown"
→ location_graph empty → BFS empty → agent stuck forever

Fix: restored _extract_location_from_text() as fallback.
"""
...
mcp = FastMCP("Text Adventure Server V11d")
...
class GameState:
    ...
    def _get_jericho_location(self) -> str:
        """Get location from Jericho API, fall back to text extraction."""
        try:
            ...
            pass
        # CRITICAL FALLBACK: extract from observation text
