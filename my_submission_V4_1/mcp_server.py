...
class GameState:
    ...
    def _get_jericho_location(self) -> str:
        ...
        Falls back to room object ID (numeric) to distinguish unnamed rooms.
        """
        try:
            loc_obj = self.env.env.get_player_location()
            name = str(loc_obj)
            if name and name != "None" and len(name) < 80:
                return name
            # Unnamed room: use the object number as unique ID
            try:
                room_id = loc_obj.num
    ...
    def compute_frontier_path(self) -> list[str]:
        ...
        def is_frontier(loc: str) -> bool:
            ...
            untried = self.ALL_DIRECTIONS - confirmed - truly_failed
...
def suggest_actions() -> str:
    ...
    EXAMINABLE = [
        ...
        "curtain","rug","table","pedestal","altar","gnome","troll","painting",
        "window","grate","ladder","crack","hole","passage","tunnel","pig",
    ]
