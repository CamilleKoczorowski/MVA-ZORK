        ...
        for step in range(1, max_steps + 1):
            ...
            if self._autopilot_queue:
                ...
                if new_location != location:
                    ...
                    # Direction had no effect — abort the rest of this path
                    if new_obs.strip() == self._last_obs.strip():
                        if verbose:
                            print(f"[AUTOPILOT] Direction '{direction}' had no effect — aborting path")
                        self._autopilot_queue.clear()
            ...
            if (has_navigate and self.memory.is_stagnant(threshold=15)
                    and not self._autopilot_queue
                    and self._navigate_fail_count < 5):  # don't retry if BFS keeps failing
                try:
