...
SYSTEM_PROMPT = """You play a text adventure. Goals: (1) explore rooms, (2) interact with objects to score points.

SCORING: Examining objects often reveals items or triggers events. Taking items is almost always good.
If you see fountain, curtain, picture, shelf, pig, gnome, statue → EXAMINE it.
...
PRIORITY:
1. Untried directions → try one (read the text for which exits exist)
2. All directions tried → EXAMINE every object in the room
3. All objects examined → TAKE anything you can
4. Truly stuck → navigate_to_frontier
...
COMMANDS: north/south/east/west/up/down/etc, take X, examine X, open X, look"""
...
class StudentAgent:
    ...
    def _build_prompt(self, obs: str, location: str, loc: LocationState) -> str:
        ...
        if loc.failed:
            parts.append("FORBIDDEN: " + ", ".join(sorted(loc.failed)))

        parts.append("\n=== OBSERVATION ===\n" + clean)
        parts.append("\nLocation: " + location + " | Score frozen: " + str(self.memory._no_score_steps) + " steps")

        if loc.tried:
            parts.append("Already tried: " + ", ".join(sorted(loc.tried)[:12]))

        # Extract nouns for hints
        nouns_to_check = ["fountain", "statue", "picture", "curtain", "shelf", "pig",
                          "gnome", "door", "torch", "lamp", "key", "bowl", "coin",
                          "tin", "block", "crack", "hole", "stairs", "tunnel", "stream"]
        clean_lower = clean.lower()
        unexamined = [n for n in nouns_to_check
                      if n in clean_lower and ("examine " + n) not in loc.tried]
        if unexamined:
            parts.append("OBJECTS TO EXAMINE: " + ", ".join(unexamined))

        if self.memory.is_stagnant(8):
            parts.append("SCORE FROZEN — examine objects or take items!")
        if loc.steps_here >= self.MAX_STEPS_ROOM:
            parts.append(str(loc.steps_here) + " steps here — leave or interact with objects")
