"""
MCP Server V11e — Critical fixes

KEY FIX: When an action fails, KEEP the previous location name.
Before: "north" fails in Statue Room → obs has no room name → "Unknown"
        → agent thinks we moved → creates new room state → loses context
After:  "north" fails in Statue Room → still "Statue Room"
        → failed_actions properly tracked → never retried

Also: removed reverse edge inference (Gemini's valid point about non-Euclidean maps).
"""
...
mcp = FastMCP("Text Adventure Server V11e")
...
class GameState:
    def __init__(self, game: str = "lostpig"):
        ...
        self.current_location: str = self._detect_location(self.state.observation)
        self.previous_location: str = ""
        self.previous_score: int = 0
        # Track the last NAMED (non-Unknown) location
        self._last_named_location: str = self.current_location

    def _detect_location(self, obs: str) -> str:
        """Get location: Jericho API first, then text extraction fallback."""
        try:
            ...
            pass
        return self._extract_location_from_text(obs)

    def _extract_location_from_text(self, obs: str) -> str:
        """Extract room name from first short capitalized line."""
        MSG = [
    ...
    def take_action(self, action: str) -> dict:
        ...
        # Detect new location
        detected_loc = self._detect_location(obs_after)

        # *** KEY FIX ***: If action failed or obs barely changed → we DIDN'T move
        # Keep previous location instead of "Unknown"
        if is_explicit_failure or (not obs_changed and not score_changed):
            new_location = prev_location
            location_changed = False
            is_failed = True
        elif detected_loc == "Unknown" and not obs_changed:
            # Obs didn't change, can't detect room → stayed put
            new_location = prev_location
            location_changed = False
            is_failed = True
        else:
            # If we detected a named room, use it
            # If Unknown but obs DID change, we probably moved
            if detected_loc != "Unknown":
                new_location = detected_loc
            elif self._last_named_location != prev_location:
                # We moved but can't name it → Unknown
                new_location = detected_loc
            else:
                new_location = detected_loc
        ...
        # Track failed actions for NAMED locations
        ...
        # NO reverse edge inference (non-Euclidean geometry in Z-machine games)
        ...
        # Update named location tracker
        if new_location != "Unknown":
            self._last_named_location = new_location
