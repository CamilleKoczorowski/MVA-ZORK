...
class GameState:
    ...
    def get_jericho_valid_actions(self) -> list[str]:
        ...
        Prof's pattern: self.env.get_valid_actions()
        Includes stderr suppression for spaCy noise (Claude Code suggestion).
        """
        import sys, io
        SKIP = {"", "yes", "no", "again", "oops", "undo", "restart", "quit", "score", "verbose"}
        # Try primary: direct call on TextAdventureEnv (prof's pattern)
        try:
            ...
            pass
        # Try secondary: inner env (our original approach)
        try:
            old_stderr = sys.stderr
            sys.stderr = io.StringIO()
            try:
