...
class GameState:
    ...
    def get_jericho_valid_actions(self) -> list[str]:
        """
        Ask Jericho for the list of valid actions in the current state.
        Returns e.g. ['north', 'east', 'take lamp', 'open mailbox', ...]
        Filters out trivial/meta actions.
        """
        try:
            valid = self.env.env.get_valid_actions()
            # Filter out empty strings and meta-commands
