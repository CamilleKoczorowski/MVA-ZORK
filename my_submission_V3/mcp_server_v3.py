...
class GameState:
    ...
    def take_action(self, action: str) -> dict:
        ...
        if not obs_changed and not score_changed:
...
def suggest_actions() -> str:
    ...
    # 1. Directions not yet tried from this location (not failed, not mapped)
    ALL_DIRECTIONS = [
    ...
    untried_dirs = [
        ...
        if d not in failed and d not in known_exits
    ]
    if untried_dirs:
        suggestions.append(f"Untried directions: {', '.join(untried_dirs[:6])}")

    # 2. Objects in observation not yet examined
    EXAMINABLE_KEYWORDS = [
    ...
    examined = failed  # actions that had no effect include repeated examines
    found_objects = [
        kw for kw in EXAMINABLE_KEYWORDS
        if kw in obs and f"examine {kw}" not in examined
    ]
    if found_objects:
